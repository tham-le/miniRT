...
/*   Updated: 2023/10/27 22:52:52 by thi-le           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "miniRT.h"

static int	err_cone(char **elems, t_data *data)
{
	if (ft_tabsize(elems) < 6)
		return (printf("Error: line %d: you should enter \
at least 5 infos for each cone\n", data->nb_lines), 1);
	...
	if (check_float(elems[3]) || check_float(elems[4]))
		return (printf("Error: line %d: the height/diameter \
